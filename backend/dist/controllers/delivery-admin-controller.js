...
export async function getDeliveryAdminStats(_req, res) {
    try {
        ...
        const partnerUserIds = partnersWithCompleted
            .map((row) => row.deliveryAgent?.userId)
...
export async function listDeliveryPartners(req, res) {
    try {
        const rawRegistrationStatus = typeof req.query.registrationStatus === "string"
        ...
        const completedMap = completedByRider.reduce((acc, row) => {
        ...
        const riderProfiles = ids.length
            ...
            : [];
        const riderProfileByUserId = new Map(riderProfiles.map((profile) => [profile.userId, profile.id]));
        const activeDeliveries = riderProfiles.length
            ? await prisma.delivery.findMany({
                where: {
                    deliveryAgentId: {
                        in: riderProfiles.map((profile) => profile.id),
                    },
                    status: {
            ...
            : [];
        const activeDeliveryByRiderProfileId = new Map();
