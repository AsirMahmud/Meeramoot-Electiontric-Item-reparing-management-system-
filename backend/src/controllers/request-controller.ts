...
export async function createRepairRequest(req: AuthedRequest, res: Response) {
  try {
    ...
    const created = await prisma.$transaction(async (tx) => {
      if ((addressMode === "MANUAL" || addressMode === "MAP") && chosenAddress.trim() && user) {
...
Pickup address: ${chosenAddress || "Not provided"}`,
          ...
          status: matchedShop ? RequestStatus.PENDING : (biddingMode ? RequestStatus.BIDDING : RequestStatus.PENDING),
        },
      });
