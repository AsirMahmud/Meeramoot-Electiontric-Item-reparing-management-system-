...
async function processSuccessLikeCallback(req: Request, res: Response, source: "success" | "ipn") {
  try {
    ...
    const callbackStatus = readCallbackField(req, "status").toUpperCase();
    ...
    const validated = isSslValidationSuccess(validationResponse);
    const callbackLooksSuccessful = callbackStatus === "VALID" || callbackStatus === "VALIDATED";

    if (!validated && !callbackLooksSuccessful) {
      await prisma.payment.update({
        where: { id: payment.id },
        data: {
          status: "FAILED",
...
async function processFailureLikeCallback(
  ...
  try {
    ...
    return res.json({
      success: true,
      message: `Payment ${source} callback processed`,
      data: { tranId, source },
    });
  } catch (error) {
    console.error(`POST /payments/sslcommerz/${source} error:`, error);
